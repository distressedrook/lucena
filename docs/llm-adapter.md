...
> The **generic** LLM layer. The orchestrator depends ONLY on this interface — never on a provider
...
That's the whole surface the orchestrator sees. No tools, no streaming (hot-path generations return
...
- **`GeminiAdapter`** (today) — wraps `google-genai`. Maps `messages` → Gemini contents +
  ...
  `usage_metadata`. (This is what `legacy/agent/orchestrator.py` / `quick.py` already do, extracted
...
1. **The orchestrator imports the interface, never a provider.** No `google.genai` / `openai` import
...
2. **Model + params are config**, resolvable per flow (e.g. a cheap model for `coach`, a slightly
   stronger one for `grade`) — but the orchestrator only passes an `opts.model` string it got from
...
3. **Retry with backoff on 429/503** lives in the adapter (fail-fast bound so a rate-limited call
   ...
   **bounded by the caller's deadline** (attempts×backoff must not outlast the turn), and a retried
   ...
   Deciding to *stop retrying and degrade* (the breaker → truth-without-voice) is the **orchestrator's**
...
6. **Never tool-calls, never sets state.** Pure text/JSON generation over what the orchestrator hands
   it.
