...
async def main():
    ...
    print("GROUNDING refutation line:")
    print("  "+[l for l in facts.split(chr(10)) if "refutes" in l][0])
    for i in range(3):
        c=await llm.generate([Message("system",VerdictPrompt.system(correct=False)),Message("user",VerdictPrompt.prompt(attempt="Kg2",facts=facts))],GenerateOptions(model="gemini-flash-lite-latest",schema={"type":"object"},max_tokens=300,temperature=0.4))
