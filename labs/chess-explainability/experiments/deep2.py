...
inp=Input(kind="move",uci="e5c6",fen=FEN,san="Nxc6+")
async def main():
    facts=await h._move_facts(inp, False, g, bit)
    print("=== GROUNDING ===\n"+facts+"\n")
    for i in range(2):
        c=await llm.generate([Message("system",VerdictPrompt.system(correct=False,player_color=color)),Message("user",VerdictPrompt.prompt(attempt="Nxc6+",facts=facts))],GenerateOptions(model="gemini-flash-lite-latest",schema={"type":"object"},max_tokens=450,temperature=0.4))
        print(f"=== OUTPUT [{i}] ===\n"+(c.json or {}).get("text","")+"\n")
asyncio.run(main())
