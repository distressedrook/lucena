...
async def verdict(ctx,h,FEN,uci,san,T=None):
    color=_color_to_move(FEN)
    g=tiered_bit_grounding(ctx.analyze_and_show(FEN,focus="analysis",board_push=False),T or {})
    bit=types.SimpleNamespace(spec=types.SimpleNamespace(params={"tree":T})) if T else None
    inp=Input(kind="move",uci=uci,fen=FEN,san=san)
    facts=await h._move_facts(inp,False,g,bit)
    print(f"=== {san} GROUNDING ===\n"+facts)
    c=await llm.generate([Message("system",VerdictPrompt.system(correct=False,player_color=color)),Message("user",VerdictPrompt.prompt(attempt=san,facts=facts))],GenerateOptions(model="gemini-flash-lite-latest",schema={"type":"object"},max_tokens=450,temperature=0.4))
    print(f"=== {san} OUTPUT ===\n"+(c.json or {}).get("text","")+"\n")
async def main():
    ...
    await verdict(ctx,h,'k7/2K5/1P6/8/7p/1rR4p/7P/8 w - - 0 7','c3c1','Rc1')  # SHORT line (Rc3+ Rxc3)
asyncio.run(main())
