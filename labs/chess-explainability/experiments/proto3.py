...
import re
def _trim(s):
    return re.sub(r";\s*\w+.s least active piece is the [^;.]+", "", s)
def assemble_v2(board, best_score, pos, facts, thresh=50):
    ...
    if not ranked:
        ...
        if is_comp:
            lines.append(f"{other}'s compensation: " + "; ".join(_trim(terms[t]['standing']) for t in opp) + ".")
        else:
            for t in ranked[1:]:                 # eval is decisive → the others are just secondary context
                lines.append(_cap(_trim(terms[t]['standing'])) + ".")
