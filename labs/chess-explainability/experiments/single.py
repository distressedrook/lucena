...
from lucena_backend.coaching.grounding import tiered_bit_grounding, _invented_moves
...
inp=Input(kind="move",uci="c3c4",fen=FEN,san="Rc4"); color=_color_to_move(FEN)
async def main():
    facts=await h._move_facts(inp, False, g, bit)
    print("=== GROUNDING FED ===\n"+facts)
    c=await llm.generate([Message("system",VerdictPrompt.system(correct=False,player_color=color)),
    ...
    text=(c.json or {}).get("text","")
    print("\n=== SINGLE RAW GEMINI FLASH OUTPUT ===\n"+text)
    print("\n=== guard verdict ===\ninvented moves:", _invented_moves(text, facts, "Rc4"))
asyncio.run(main())
