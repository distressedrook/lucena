...
async def main():
    ...
    print("=== GROUNDING ===\n"+facts+"\n")
    for i in range(2):
        c=await llm.generate([Message("system",VerdictPrompt.system(correct=False,player_color=color)),Message("user",VerdictPrompt.prompt(attempt="Nxc6+",facts=facts))],GenerateOptions(model="gemini-flash-lite-latest",schema={"type":"object"},max_tokens=400,temperature=0.4))
        print(f"=== OUTPUT [{i}] ===\n"+(c.json or {}).get("text","")+"\n")
asyncio.run(main())
